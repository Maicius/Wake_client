...
public class AlarmKlaxon extends Service {
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case KILLER:
                    Log.v("maicius", "*********** Alarm killer triggered ***********");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (alarm == null) {
            Log.v("maicius", "AlarmKlaxon failed to parse the alarm from the intent");
            stopSelf();
            return START_NOT_STICKY;
        }
    ...
    private void play(Alarm alarm) {
        ...
        Log.v("maicius", "AlarmKlaxon.play() " + alarm.id + " alert " + alarm.alert);
        ...
        if (!alarm.silent) {
            ...
            if (alert == null) {
                ...
                Log.v("maicius", "Using default alarm: " + alert.toString());
            }
            ...
            mMediaPlayer.setOnErrorListener(new OnErrorListener() {
                public boolean onError(MediaPlayer mp, int what, int extra) {
                    Log.v("maicius", "Error occurred while playing audio.");
                    mp.stop();
                    mp.release();
                    mMediaPlayer = null;
                    return true;
            ...
            try {
                ...
                if (mTelephonyManager.getCallState()
                    ...
                    Log.v("maicius", "Using the in-call alarm");
                ...
                Log.v("maicius", "Using the fallback ringtone");
                ...
                try {
                    ...
                    Log.v("maicius", "Failed to play fallback ringtone"+ex2);
                }
            }
        }
     ...
     */
    public void stop() {
        Log.v("maicius", "AlarmKlaxon.stop()");
        if (mPlaying) {
            mPlaying = false;
